...
for match in matches:
    ...
    if ((home_team_name==home_team_required) and (away_team_name==away_team_required) 
        or (home_team_name==away_team_required) and (away_team_name==home_team_required)):
        match_id_required = match['match_id']
print(home_team_required + ' vs ' + away_team_required + ' has id:' + str(match_id_required))

#Write out only Sweden's results
for match in matches:
    home_team_name=match['home_team']['home_team_name']
    away_team_name=match['away_team']['away_team_name']
    home_score=match['home_score']
...
#Exercise: 
#1, Edit the code above to print out the result list for the Mens World cup - done
#2, Edit the code above to find the ID for England vs. Sweden - id:8651
